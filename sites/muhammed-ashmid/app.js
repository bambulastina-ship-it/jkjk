/* Muhammed Ashmid — interaction layer.
...
(function () {
  ...
  var FS = [
    ...
    ' float w1=fbm(q*1.7+vec2(-t*.026,t*.022));',
    ...
    ' vec3 champ=vec3(.784,.616,.400);',
    ' vec3 c=mix(base,coal,smoothstep(.22,.86,f));',
    ' float g=smoothstep(.58,.95,w2)*smoothstep(.95,.30,uv.y);',
    ' c=mix(c,champ,g*.26);',
    ' c+=champ*.075*smoothstep(1.05,.0,length(uv-vec2(.50,.88)));',
